  ...
  <div class="second">
    ...
    <div style="border: 1px solid #ccc; border-radius: 5px; background: #fefefe; padding: 5px; margin: 0 0 10px 0;">
      <g:formRemote name="formRemote" url="[controller:'comment', action:'updatelist']" update="results" before="showspinner('#results')">
        <table>
          <tr>
            <td class="bold" valign="top" style="width: 100px;">Zeitraum:</td>
            <td><g:textField class="datepicker" name="dateFrom" value="${formatDate(date: new Date() - 7, format: 'dd. MM. yyyy')}"/> bis <g:textField class="datepicker" name="dateTo" value="${formatDate(date: new Date(), format: 'dd. MM. yyyy')}"/></td>
          </tr>
          <tr>
            <td class="bold" valign="top">Aus:</td>
            <td>
