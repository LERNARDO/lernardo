<div class="gray" style="width: 200px; float: left;">
  <g:if test="${currentEntity.id == entity.id}">
    <g:if test="${!unit.confirmed}">
      <g:remoteLink action="editRecord" update="unit-${i}" id="${unit.id}" params="[i: i, entity: entity.id]"><img src="${g.resource(dir:'images/icons', file:'icon_edit2.png')}" alt="${message(code:'edit')}" align="top"/></g:remoteLink> <g:remoteLink action="removeRecord" update="unit-${i}" id="${unit.id}" params="[entity: entity.id]" before="if(!confirm('${message(code:'delete.warn')}')) return false"><img src="${g.resource(dir:'images/icons', file:'cross.png')}" alt="${message(code:'delete')}" align="top"/></g:remoteLink>
    </g:if>
  </g:if>
