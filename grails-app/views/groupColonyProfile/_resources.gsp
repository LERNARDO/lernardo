<g:if test="${resources}">
  <g:each in="${resources}" var="resource">
    <div style="border: 1px solid #ccc; margin-top: 5px; border-radius: 5px; background: #fefefe; padding: 5px;">
      %{--<ul>
        <li><span class="bold"><g:message code="name"/>:</span> <g:link controller="resourceProfile" action="show" id="${resource.id}">${resource.profile.decodeHTML()}</g:link> <erp:accessCheck types="['Betreiber']"><g:remoteLink action="removeResource" update="resources2" id="${group.id}" params="[resource: resource.id]" before="if(!confirm('${message(code:'delete.warn')}')) return false"><img src="${g.resource(dir:'images/icons', file:'icon_remove.png')}" alt="${message(code: 'remove')}" align="top"/></g:remoteLink></erp:accessCheck></li>
      ...
      <ul>
        <li><erp:accessCheck types="['Betreiber']"><g:remoteLink action="moveUp" update="resources2" id="${resource.id}" params="[colony: group.id]"><img src="${g.resource(dir: 'images/icons', file: 'arrow_up.png')}" alt="${message(code:'up')}" align="top"/></g:remoteLink><g:remoteLink action="moveDown" update="resources2" id="${resource.id}" params="[colony: group.id]"><img src="${g.resource(dir: 'images/icons', file: 'arrow_down.png')}" alt="${message(code:'down')}" align="top"/></g:remoteLink><g:remoteLink action="removeResource" update="resources2" id="${group.id}" params="[resource: resource.id]" before="if(!confirm('${message(code:'delete.warn')}')) return false"><img src="${g.resource(dir:'images/icons', file:'icon_remove.png')}" alt="${message(code: 'remove')}" align="top"/></g:remoteLink></erp:accessCheck> ${resource.profile.amount}x <g:link controller="resourceProfile" action="show" id="${resource.id}">${resource.profile}</g:link>, <g:message code="resource.info" args="[message(code: 'resourceclass.'+resource.profile.classification), resource.profile.costs, grailsApplication.config.currencySymbol, message(code: 'costsUnit.'+resource.profile.costsUnit)]"/></li>
