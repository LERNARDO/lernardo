...
<g:if test="${entity.profile.favorites}">
  <ul>
    <g:each in="${entity.profile.favorites}" var="fav">
      ...
      <li class="icon-star"><g:link controller="${favorite.type.supertype.name + 'Profile'}" action="show" id="${favorite.id}">${favorite.profile.decodeHTML()}</g:link> <g:message code="short.${favorite.type.supertype.name}"/></li>
    </g:each>
  </ul>
</g:if>
<g:else>
